...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        // 初始化文字
        [self setTitle:MJRefreshHeaderStatePullingText forState:MJRefreshFooterStateIdle];
...
- (void)adjustStateWithContentOffset
{
    ...
    if (_scrollView.mj_insetT + _scrollView.mj_contentSizeH > _scrollView.mj_h) { // 内容超过一个屏幕
        ...
        if (_scrollView.mj_offsetY > _scrollView.mj_contentSizeH - _scrollView.mj_h + self.mj_h * self.appearencePercentTriggerAutoRefresh + _scrollView.mj_insetB - self.mj_h + 150) {
            // 当底部刷新控件完全出现时，才刷新
            [self beginRefreshing];
        }
    }
